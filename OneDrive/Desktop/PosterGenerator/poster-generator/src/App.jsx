...
const App = () => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        setUploadedTemplate({
          ...
          colors: { text: '#000000', shadow: '#ffffff', gradient: ['#FFD700', '#FF6B35'] },
        });
        setTpl(TEMPLATE_OPTIONS.length); // Select the uploaded template automatically
      };
      reader.readAsDataURL(file);
    }
  };

  // Combine built-in and uploaded templates together for rendering and usage
  const templateOptions = [...TEMPLATE_OPTIONS];
  if (uploadedTemplate) templateOptions.push(uploadedTemplate);
  ...
  const drawPoster = useCallback(() => {
    ...
    const currentTemplate = templateOptions[tpl];
    const template = new Image();

    if (uploadedTemplate && tpl === templateOptions.length - 1) {
      template.src = uploadedTemplate.file; // base64 uploaded image
    } else {
      template.src = `/templates/${currentTemplate.file}`; // built-in template
    }
    ...
    template.onload = () => {
      ...
      if (shopName) {
        drawText(
          ...
          currentTemplate.colors.text,
          currentTemplate.colors.shadow,
          textWidth
        );
      }
      ...
      if (offer) {
        drawGradientText(
          ...
          currentTemplate.colors.gradient,
          textWidth
        );
      }
    ...
    template.onerror = () => {
      ...
      canvas.width = 800;
      canvas.height = 1200;
      const currentTemplateFallback = templateOptions[tpl];
      const gradient = ctx.createLinearGradient(0, 0, 0, canvas.height);
      gradient.addColorStop(0, currentTemplateFallback.colors.gradient[0]);
      gradient.addColorStop(1, currentTemplateFallback.colors.gradient[1]);
      ctx.fillStyle = gradient;
  ...
  }, [tpl, shopName, offer, address, uploadedTemplate, templateOptions]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <div className="container mx-auto px-4 py-6 max-w-7xl">
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-6 text-center bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">
          Local Store Poster Designer
        </h1>

        {/* Template thumbnails including uploaded template */}
        <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-4 scrollbar-hide">
          {templateOptions.map((template, i) => (
            <div
              key={template.file + i}
              className={`w-24 sm:w-32 shrink-0 cursor-pointer border-2 rounded-xl overflow-hidden transition-all duration-200 ${
                tpl === i ? "border-cyan-400 shadow-lg shadow-cyan-400/25" : "border-gray-600 hover:border-gray-400"
              }`}
              onClick={() => setTpl(i)}
            >
              <img
                src={template.file.startsWith("data:") ? template.file : `/templates/${template.file}`}
                alt="template"
        ...
        {/* Upload input to add custom template */}
        <div className="mt-3">
          <label className="block text-sm font-medium mb-2 text-gray-200">Add Your Own Template</label>
          <input
            type="file"
            accept="image/*"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="order-2 lg:order-1">
            <canvas
              ref={canvasRef}
              className="w-full max-w-md mx-auto lg:max-w-none shadow-2xl rounded-2xl bg-white/10 backdrop-blur-sm"
            />
          </motion.div>
                  ...
                  <div className="space-y-2">
                    ...
                    {locationStatus === 'idle' && (
                      <button
                        ...
                        className="w-full text-sm bg-white/10 hover:bg-white/20 text-cyan-400 py-2 px-3 rounded-lg border border-cyan-400/30 hover:border-cyan-400/50 transition-all duration-200"
                      >
                        📍 Use My Location
                      </button>
                    )}
                    {locationStatus === 'requesting' && (
                      <div className="text-center text-sm text-gray-300 py-2">🔄 Getting your location...</div>
                    )}
                    {locationStatus === 'denied' && (
                      <div className="text-center text-sm text-red-400 py-2">❌ Location access denied or unavailable</div>
                    )}
                  </div>
                </div>
                <button
